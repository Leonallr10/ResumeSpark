...
export function LatexResumeTailorApp() {
  ...
  const canSubmit =
    ...
    !suggesting;
  const compilerUnavailable = compilerStatus?.available === false;
  const canCompilePdf =
    resumeSections.length > 0 &&
    latexCode.trim().length > 0 &&
    !compilerUnavailable;
...
const ResumePreview = forwardRef<HTMLDivElement, ResumePreviewProps>(
  function ResumePreviewInner({ sections, zoom }, ref) {
    return (
      <div className="mx-auto w-full max-w-[820px]">
        <div
          ref={ref}
          className="resume-page resume-preview-page px-12 py-10"
...
function PreviewSection({
  ...
  if (section.title === "Header") {
    ...
    const contacts = contactLines.map((line) => line.text).filter(Boolean);

    return (
      <header className="mb-5 text-center">
        {nameLine ? (
          <h1 className="text-3xl font-normal uppercase leading-none tracking-[0.22em]">
            {nameLine.text}
          </h1>
        ) : null}
        {locationLine ? (
          <p className="mt-1 text-sm leading-4 text-slate-900">{locationLine.text}</p>
        ) : null}
        {contacts.length > 0 ? (
          <div className="mx-auto mt-1 flex max-w-[560px] flex-wrap justify-center gap-x-7 gap-y-0 text-xs leading-4 text-slate-900">
            {contacts.map((contact, index) => (
              <span key={`${contact}-${index}`} className="whitespace-nowrap">
                {contact}
              </span>
            ))}
          </div>
  ...
  return (
    <section className="mb-4 break-inside-avoid">
      <div className="mb-2 flex items-center gap-2 border-b border-slate-900 pb-1">
        <h2 className="text-sm font-bold uppercase tracking-normal">{section.title}</h2>
      </div>
      ...
      {line.kind === "projectHeading" || line.kind === "subheading" ? (
        <div>
          <div className="flex items-start justify-between gap-4 text-sm">
            ...
            {line.rightText ? (
              <span className="shrink-0 text-right text-xs font-semibold text-slate-700">
                {line.rightText}
              </span>
            ) : null}
          </div>
          {line.secondaryText ? (
            <p className="text-xs italic leading-5 text-slate-700">{line.secondaryText}</p>
          ) : null}
        </div>
      ) : line.kind === "bullet" ? (
        <div className="grid grid-cols-[14px_minmax(0,1fr)] gap-1 text-xs leading-5">
        ...
        <p className="text-xs leading-5">{line.text}</p>
      )}
    </div>
  );
}
